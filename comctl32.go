...
func ImageList_Add(himl HIMAGELIST, hbmImage, hbmMask HBITMAP) int32 {
	ret, _, _ := syscall.SyscallN(imageList_Add.Addr(),
		uintptr(himl),
		uintptr(hbmImage),
		uintptr(hbmMask))
...
func ImageList_AddMasked(himl HIMAGELIST, hbmImage HBITMAP, crMask COLORREF) int32 {
	ret, _, _ := syscall.SyscallN(imageList_AddMasked.Addr(),
		uintptr(himl),
		uintptr(hbmImage),
		uintptr(crMask))
...
func ImageList_Create(cx, cy int32, flags uint32, cInitial, cGrow int32) HIMAGELIST {
	ret, _, _ := syscall.SyscallN(imageList_Create.Addr(),
		uintptr(cx),
		uintptr(cy),
		uintptr(flags),
		uintptr(cInitial),
...
func ImageList_Destroy(himl HIMAGELIST) bool {
	ret, _, _ := syscall.SyscallN(imageList_Destroy.Addr(),
		uintptr(himl))
...
func ImageList_DrawEx(himl HIMAGELIST, i int32, hdcDst HDC, x, y, dx, dy int32, rgbBk COLORREF, rgbFg COLORREF, fStyle uint32) bool {
	ret, _, _ := syscall.SyscallN(imageList_DrawEx.Addr(),
		uintptr(himl),
		uintptr(i),
		uintptr(hdcDst),
		uintptr(x),
...
func ImageList_ReplaceIcon(himl HIMAGELIST, i int32, hicon HICON) int32 {
	ret, _, _ := syscall.SyscallN(imageList_ReplaceIcon.Addr(),
		uintptr(himl),
		uintptr(i),
		uintptr(hicon))
...
func InitCommonControlsEx(lpInitCtrls *INITCOMMONCONTROLSEX) bool {
	ret, _, _ := syscall.SyscallN(initCommonControlsEx.Addr(),
...
func LoadIconMetric(hInstance HINSTANCE, lpIconName *uint16, lims int32, hicon *HICON) HRESULT {
	...
	ret, _, _ := syscall.SyscallN(loadIconMetric.Addr(),
		uintptr(hInstance),
